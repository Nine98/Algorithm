...
//服务器购买信息
typedef struct{
    int count;//购买的一种服务器数量
    vector<int> sids;//同种服务器的id序列
} purchase;
...
vector<choice_server> choice_serverCoreEqRam;
...
void loadServer(string &serverType,string &cpuCores,string &memorySize,
    ...
    if(_cpuCores == _memorySize){
        choice_serverCoreEqRam.push_back(server);
    }
}
...
bool cmpRunningServerCoreAndRam(running_server a, running_server b){ // sort rule of migration servers: rest core and ram , more is prior
    return (weight*(a.a.core + a.b.core) + a.a.ram + a.b.ram) > (weight*(b.a.core + b.b.core) + b.a.ram + b.b.ram);
}
...
bool cmpVmsCoreAndRam(int a, int b){
    return (weight*vm2deploy[a].core + vm2deploy[a].ram) < (weight*vm2deploy[b].core + vm2deploy[b].ram);
}
...
bool cmpAddRequests(ReqCmd a, ReqCmd b){ // vm request rest days sort
    return (weight*a.core + a.ram) > (weight*b.core + b.ram);
}
...
double inline running_servers_weight(vector<running_server>&  running_servers){ 
    // return core / ram weight running_servers
    double coreNums = 0, ramNums = 0;
    for (int i = 0; i < running_servers.size(); i++){
        coreNums += (running_servers[i].a.core + running_servers[i].b.core);
        ramNums += (running_servers[i].a.ram + running_servers[i].b.ram);
    }
    if(coreNums == 0 || ramNums == 0){
        return 1.0;
    }
    return coreNums / ramNums;
}
...
int main()
{
    ...
    for (int day = 0; day < days; day++){
        restDays = days - day;
        // weight = running_servers_weight(running_servers);
        if(day < days/3){
        ...
        unordered_map<int, int> serverId2index; // 服务器 id 到服务器 下标

        // 第一天，购买价格最低的前十台服务器
        // if(day == 0){
        //     sort(choice_serverCoreEqRam.begin(), choice_serverCoreEqRam.end(), cmpServerCostPowerCost);
        //     int j = 0;//choice_serverCoreEqRam.size() / 5;
        //     for (int i = 0; i < 3 && i < choice_serverCoreEqRam.size(); i++){
        //         choice_server server = choice_serverCoreEqRam[j];
        ...
        // 第一天，购买价格最低的前十台服务器
        if(day <= 1){
            for (int i = 0; i < 10 && i < choice_servers.size(); i++){
        ...
        for (int sIndex = 0; sIndex < runningServersNum; sIndex++){
            ...
            // 遍历该服务器的所有虚拟机，查看是否为所有虚拟机都能符合条件 
            
            int dIndex = runningServersNum-1;
            for (int vIndex = 0; vIndex < vmsNums; vIndex++){
                deploy vm = vm2deploy[vms[vIndex]]; // 原始部署信息
                // 遍历所有的目的服务器，查看是否有服务器能够迁移该虚拟机
                ...
                while(dIndex > sIndex){
                    ...
                    else if(vm.dnode == 0){ // 单节点
                        if(running_servers[dIndex].a.core >= vm.core && 
                            running_servers[dIndex].a.ram >= vm.ram){ // 目的服务器的A节点
                            ...
                            migrationMax--;
                            // 记录迁移信息
                            migration mig = {vm.id, running_servers[dIndex].id , 1, 0}; // 目的服务器A节点 单节点
                            migs.push_back(mig);
                            break;
                        }
                        else if(running_servers[dIndex].b.core >= vm.core && 
                                running_servers[dIndex].b.ram >= vm.ram){ // 目的服务器的B节点
                            ...
                            migrationMax--;
                            // 记录迁移信息
                            migration mig = {vm.id, running_servers[dIndex].id , 0, 0}; // 目的服务器B节点 单节点
                            migs.push_back(mig);
                            break;
                        }
                        else{
        ...
        if(flag == 0 && migCores / runningServersNum > 0.2 && migCores / runningServersNum > 0.2){
            int ts = 0;
            int start = 0;
                ...
                if(sid == slen){ // purchase server
                    if(reqs[day][rid].mode == 0){ // single node   --- purchase server
                        for (int i=0; i < choice_servers.size();i++){
                            if(choice_servers[i].a.core >= 2 * reqs[day][rid].core && 
                                choice_servers[i].a.ram >= 2 * reqs[day][rid].ram){
                                deploy vm = create_vm(reqs[day][rid],1,0);
                                vm2server[vm.id] = id;
                                vm2deploy[vm.id] = vm;
